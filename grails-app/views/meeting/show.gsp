...
<html>
	...
	<body>
		<div id="show-meeting" class="content scaffold-show" role="main">
			<h1>${meetingInstance?.title}</h1>
			<g:if test="${flash.message}">
			<div class="message" role="status">${flash.message}</div>
			</g:if>
			<p>Presented by ${meetingInstance?.speaker} on <g:formatDate format="EEEE, MMMM d, yyyy" style="LONG" date="${meetingInstance?.meetingDate}" /> from ${meetingInstance?.startTime} to ${meetingInstance?.endTime}. The location is <g:link controller="location" action="show" id="${meetingInstance?.location?.id}">${meetingInstance?.location?.name?.encodeAsHTML()}</g:link>.</p>
			<p><g:link action="icalendar" id="${meetingInstance.id}">Add To My Calendar</g:link></p>
